...
#     return x / norm.to(x.dtype)
#     #return (x.to(torch.float32) / norm.to(torch.float32)).to(x.dtype)
...
@persistence.persistent_class
class MPConv(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel,force_normalization,use_gan): #=True):
        super().__init__()
        self.out_channels = out_channels
        self.weight = torch.nn.Parameter(torch.randn(out_channels, in_channels, *kernel))
...
class Block(torch.nn.Module):
    def __init__(self,
        ...
        self.conv_res0 = MPConv(out_channels if flavor == 'enc' else in_channels, out_channels, kernel=[3,3],force_normalization=force_normalization,use_gan=use_gan)
        self.emb_linear = MPConv(emb_channels, out_channels, kernel=[],force_normalization=force_normalization,use_gan=use_gan)
        self.conv_res1 = MPConv(out_channels, out_channels, kernel=[3,3],force_normalization=force_normalization,use_gan=use_gan)
        self.conv_skip = MPConv(in_channels, out_channels, kernel=[1,1],force_normalization=force_normalization,use_gan=use_gan) if in_channels != out_channels else None
        self.attn_qkv = MPConv(out_channels, out_channels * 3, kernel=[1,1],force_normalization=force_normalization,use_gan=use_gan) if self.num_heads != 0 else None
        self.attn_proj = MPConv(out_channels, out_channels, kernel=[1,1],force_normalization=force_normalization,use_gan=use_gan) if self.num_heads != 0 else None
