<div align="center">

<h1>Generative Data Augmentation via Diffusion Distillation, Adversarial Alignment and Importance Reweighting</h1>

<!-- Ruyi An, Yewen Li, Xu He, Pengjie Gu, Mengchen Zhao, Dong Li, Jianye HAO, Chaojie Wang, Bo An, Mingyuan Zhou -->

<div>
    Ruyi An&emsp;
    Haicheng Huang&emsp;
    Hunagjie Zheng&emsp;
    Mingyuan Zhou
</div>

<div>
    🎈 <strong>NeurIPS 2025</strong>
</div>

<div>
    <h4 align="center">
        • <a href="https://openreview.net/pdf?id=fGOZjuzvJb" target='_blank'>[pdf]</a> •
    </h4>
</div>

<!-- <img src="assets/poster.png" width="800px"/> -->

</div>

## 📮 News
- Dec. 2024: We released the code. 
- Oct. 2024: Our paper is accepted at NeurIPS 2025. We will release the code in due time. Stay tuned!

## 🔝 Citation
If you find our work useful for your research, kindly consider citing our paper:
```bibtex
@inproceedings{hier_rep_rl,
  title={Generative Data Augmentation via Diffusion Distillation, Adversarial Alignment and Importance Reweighting},
  author={An, Ruyi and Huang, Haicheng and Zheng, Huangjie and Zhou, Mingyuan},
  booktitle={{NeurIPS}},
  year={2025}
}
```

## 🖖 Acknowledgement
This implementation is based on the following repositories:
- [pytorch-cifar](https://github.com/kuangliu/pytorch-cifar)
- [vision](https://github.com/pytorch/vision/blob/main/references/classification/train.py)

🫡 Salute!

## ☎️ Contact
Please feel free to reach us out at `ran003😎ntu.edu.sg` should you need any help.