...
class SIDA_EDMLoss:
    ...
    def generator_share_encoder_loss(self, true_score, fake_score, images, labels=None,
                                     augment_pipe=None, alpha=1.2, tmax=800, return_y_D=True):
        ...
        y, augment_labels = augment_pipe(images) if augment_pipe is not None else (
        images, torch.zeros(images.shape[0], 9).to(images.device))
        n = torch.randn_like(y) * sigma

        y_real = true_score(y + n, sigma, labels, augment_labels=augment_labels)

        if return_y_D:
            y_fake, y_D = fake_score(y + n, sigma, labels, augment_labels=augment_labels,
        ...
        with torch.no_grad():
            weight_factor = abs(y - y_real).to(torch.float32).mean(dim=[1, 2, 3],
    ...
    def fakescore_discriminator_share_encoder_loss(self, fake_score: torch.nn.Module,
                                                   ...
                                                   labels=None,
                                                   augment_pipe: callable = None, real_images=None):
        ...
        y_real_images, label_temp = augment_pipe(real_images) if augment_pipe is not None else (
        real_images, torch.zeros(images.shape[0], 9).to(images.device))

        logit_real = fake_score(y_real_images + n, sigma, labels, augment_labels=label_temp,
                                return_flag='encoder')

        nan_mask = torch.isnan(y_augmented).flatten(start_dim=1).any(dim=1) | torch.isnan(y_fake).flatten(
            start_dim=1).any(dim=1)

        nan_mask = nan_mask | torch.isnan(logit_fake).flatten(start_dim=1).any(dim=1) | torch.isnan(
            logit_real).flatten(start_dim=1).any(dim=1)
        if nan_mask.any():
            # Invert the nan_mask to get a mask of samples without NaNs
            non_nan_mask = ~nan_mask
            # Filter out samples with NaNs from y_real and y_fake
            ...
            y_augmented = y_augmented[non_nan_mask]
            y_fake = y_fake[non_nan_mask]
            weight = weight[non_nan_mask]

        loss_fake_score = weight * ((y_fake - y_augmented) ** 2)
    ...
    def __call__(self, fake_score, images, labels=None, augment_pipe=None):
        ...
        n = torch.randn_like(y) * sigma
        y_fake = fake_score(y + n, sigma, labels, augment_labels=augment_labels)
        nan_mask = torch.isnan(y).flatten(start_dim=1).any(dim=1) | torch.isnan(y_fake).flatten(
            start_dim=1).any(dim=1)
        if nan_mask.any():
            # Invert the nan_mask to get a mask of samples without NaNs
            non_nan_mask = ~nan_mask
