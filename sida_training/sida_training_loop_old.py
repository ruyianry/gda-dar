...
def setup_snapshot_image_grid(training_set,
    ...
    gw = np.clip(7680 // image_shapes[2], 7, 32)
    gh = np.clip(4320 // image_shapes[1], 4, 10)
...
def training_loop(
    ...
    if dist.get_rank() == 0:
        grid_size, images, labels = setup_snapshot_image_grid(training_set=dataset_obj,
                                                              ...
                                                              label_dim=train_dataset_ref.label_dim,
                                                              image_shapes=train_dataset_ref.image_shape,
    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            real_images, labels, _ = next(dataset_iterator)
            ...
            with misc.ddp_sync(G_ddp, False):
                with torch.no_grad():
                    images = G_ddp(z, init_sigma * torch.ones(z.shape[0], 1, 1, 1).to(z.device),
                                   labels, augment_labels=torch.zeros(z.shape[0], 9).to(z.device))
        ...
        with misc.ddp_sync(fake_score_ddp, False):
            for round_idx in range(num_accumulation_rounds):
                images, labels, _ = next(dataset_iterator)
        ...
        if ((snapshot_ticks is not None) and
            ...
            #     images = torch.cat([
            #         G_ema(z,
            #               init_sigma * torch.ones(z.shape[0], 1, 1, 1).to(z.device).to(z.dtype),
            #               c,
            #               augment_labels=torch.zeros(z.shape[0], 9).to(z.device).to(z.dtype)).cpu()
        ...
        if (state_dump_ticks is not None) and (
            ...
            most_recent_save = os.path.join(run_dir, f'training-state-{cur_nimg // 1000:06d}.pt')
