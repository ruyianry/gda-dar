...
from sida_training.vae_edm2_utils import load_sd_vae
...
def run_generator(G, z, c, init_sigma, opts):
    with torch.no_grad():
        ...
        if vae is not None:
            ...
            # print out shape of img and z
            print(f"img shape: {img.shape}")  # img shape: torch.Size([16, 3, 32, 32])
            print(f"z shape: {z.shape}")  # z shape: torch.Size([16, 3, 32, 32])

            # printout bias and scale
            print(f"bias shape: {bias_.shape}")
            print(f"scale shape: {scale_.shape}")
...
def generate_new_images(net,
    ...
    for label in range(net.label_dim):
        ...
        for i, batch_seeds in enumerate(rank_batches):
